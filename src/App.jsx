...
export default function QAApp() {
    ...
    <div className="flex h-screen bg-[#f4f4f5] bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-zinc-200/50 via-[#f4f4f5] to-zinc-100 text-zinc-800 font-sans overflow-hidden selection:bg-zinc-200">
      ...
      <aside className="w-60 bg-white/70 backdrop-blur-2xl border-r border-zinc-200/60 flex flex-col z-20 shadow-[4px_0_24px_rgb(0,0,0,0.03)]">
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto custom-scrollbar">
          ...
          <div className="pt-6 pb-2 px-2 text-[10px] font-bold text-zinc-400 tracking-wider uppercase">Projects</div>
          {data.projects.map(p => (
            <NavItem 
              key={p.id} 
              icon={<Folder size={16}/>} 
          ...
          <button 
            ...
            className="w-full flex items-center gap-2 px-3 py-2.5 mt-4 rounded-xl text-xs font-semibold text-zinc-500 hover:text-zinc-800 hover:bg-white/80 transition-all border border-dashed border-zinc-300 hover:border-zinc-500 hover:shadow-sm"
          >
      ...
      <main className="flex-1 flex flex-col z-10 h-screen overflow-hidden relative">
        <header className="h-14 flex items-center justify-between px-8 border-b border-zinc-200/60 bg-white/60 backdrop-blur-2xl sticky top-0 z-30 shadow-sm">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 h-[340px]">
              ...
              {pieData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    <RechartsTooltip 
                      contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.9)', backdropFilter: 'blur(8px)', border: '1px solid rgba(228, 228, 231, 0.8)', borderRadius: '12px', fontSize: '12px', boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.1), 0 8px 10px -6px rgba(0, 0, 0, 0.1)' }}
                      itemStyle={{ color: '#18181b', fontWeight: 'bold' }}
                    />
                    <Legend verticalAlign="bottom" height={24} iconSize={8} wrapperStyle={{ fontSize: '11px', color: '#52525b', fontWeight: '600' }}/>
                  </PieChart>
                </ResponsiveContainer>
              ) : (
                <div className="absolute inset-0 flex flex-col items-center justify-center text-xs text-zinc-400 font-medium">
                  <Activity size={24} className="mb-2 text-zinc-300 opacity-50"/>
                  데이터 없음
                </div>
              )}
            </div>
          ...
          <div className="col-span-1 lg:col-span-2 bg-white/80 backdrop-blur-xl border border-white shadow-[0_8px_30px_rgba(0,0,0,0.04)] rounded-2xl p-6 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-3">
              {data.runs.length === 0 ? (
                 <div className="h-full flex flex-col items-center justify-center text-xs text-zinc-400 font-medium">
                   <PlayCircle size={28} className="mb-2 text-zinc-300 opacity-50"/>
                   생성된 테스트 런이 없습니다.
                 </div>
              ) : (
                data.runs.map(run => {
                  ...
                  return (
                    <div key={run.id} onClick={() => { setActiveRunId(run.id); setCurrentView('execute_run'); }} className="group bg-white/60 border border-zinc-200/60 rounded-xl p-4 cursor-pointer hover:bg-white hover:border-zinc-400 hover:shadow-[0_4px_20px_rgba(24,24,27,0.08)] transition-all duration-300">
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold border ${run.status === 'completed' ? 'bg-zinc-100/50 text-zinc-500 border-zinc-200/60' : 'bg-zinc-100 text-zinc-700 border-zinc-300/60'}`}>
                      ...
                      <div className="mt-2">
                        ...
                        <div className="h-2 w-full bg-zinc-100 rounded-full overflow-hidden flex shadow-inner">
        ...
        <div className="max-w-xl mx-auto mt-8">
          <div className="bg-white/80 backdrop-blur-xl border border-white shadow-[0_8px_30px_rgba(0,0,0,0.04)] rounded-2xl p-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <form onSubmit={handleCreateProject} className="space-y-5">
              <div>
                <label className="block text-[11px] font-bold text-zinc-500 mb-1.5 ml-1 uppercase tracking-wider">Project Name</label>
                <input name="name" required className="w-full bg-white/50 backdrop-blur-sm border border-zinc-200/80 rounded-xl px-4 py-3 text-sm text-zinc-900 focus:outline-none focus:border-zinc-500/50 focus:ring-4 focus:ring-zinc-500/10 transition-all shadow-inner" placeholder="예: 모바일 앱 리뉴얼 v3.0" />
              </div>
              <div>
                <label className="block text-[11px] font-bold text-zinc-500 mb-1.5 ml-1 uppercase tracking-wider">Description (Optional)</label>
                <textarea name="desc" rows={4} className="w-full bg-white/50 backdrop-blur-sm border border-zinc-200/80 rounded-xl px-4 py-3 text-sm text-zinc-900 focus:outline-none focus:border-zinc-500/50 focus:ring-4 focus:ring-zinc-500/10 transition-all resize-none shadow-inner" placeholder="프로젝트의 목적이나 주요 내용을 간략히 적어주세요."></textarea>
              </div>
              <div className="flex justify-end gap-3 pt-6 mt-4">
                <button type="button" onClick={() => setCurrentView('dashboard')} className="px-5 py-2.5 rounded-xl text-xs font-bold text-zinc-500 hover:bg-zinc-100 hover:text-zinc-800 transition-all">취소</button>
                <button type="submit" className="px-6 py-2.5 rounded-xl text-xs font-bold bg-gradient-to-b from-zinc-700 to-zinc-900 hover:from-zinc-800 hover:to-black text-white shadow-[0_4px_14px_0_rgba(24,24,27,0.39)] hover:shadow-[0_6px_20px_rgba(24,24,27,0.23)] border border-zinc-700/50 transition-all">프로젝트 생성</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="flex justify-between items-start mb-6">
          <div className="flex-1">
             <div className="flex items-center gap-3 mb-1.5">
                 <h2 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-zinc-900 to-zinc-700 tracking-tight">{project.name}</h2>
                 <button onClick={() => { setEditProjectName(project.name); setIsProjectSettingsOpen(true); }} className="text-zinc-400 hover:text-zinc-800 hover:bg-zinc-200/50 p-1.5 rounded-lg transition-all">
                     <Settings size={18} />
                 </button>
             </div>
            <p className="text-zinc-500 text-[13px] font-medium">{project.description || '설명이 없습니다.'}</p>
          </div>
        </div>
        ...
        {isProjectSettingsOpen && (
            <div className="mb-6 p-5 bg-white/80 backdrop-blur-xl border border-white shadow-[0_8px_30px_rgba(0,0,0,0.06)] rounded-2xl animate-in fade-in slide-in-from-top-2 duration-300">
                ...
                <form onSubmit={handleUpdateProject} className="flex items-end gap-3">
                    <div className="flex-1">
                        <label className="block text-[11px] font-bold text-zinc-500 mb-1.5 ml-1 uppercase tracking-wider">Rename Project</label>
                        <input value={editProjectName} onChange={(e) => setEditProjectName(e.target.value)} required className="w-full bg-white/50 backdrop-blur-sm border border-zinc-200/80 rounded-xl px-4 py-2.5 text-xs text-zinc-900 focus:outline-none focus:border-zinc-500/50 focus:ring-4 focus:ring-zinc-500/10 transition-all shadow-inner" />
                    </div>
                    <button type="submit" className="px-4 py-2.5 bg-gradient-to-b from-zinc-700 to-zinc-900 hover:from-zinc-800 hover:to-black text-white rounded-xl text-xs font-bold shadow-[0_4px_14px_0_rgba(24,24,27,0.39)] border border-zinc-700/50 transition-all">저장</button>
                    ...
                    <button type="button" onClick={() => setIsProjectSettingsOpen(false)} className="px-4 py-2.5 text-zinc-500 hover:bg-zinc-100 rounded-xl text-xs font-bold transition-all">취소</button>
                </form>
            </div>
        )}

        <div className="flex items-center gap-3 mb-8">
          <button onClick={() => setCurrentView('create_suite')} className="flex items-center gap-2 px-4 py-2.5 bg-white/80 backdrop-blur-md text-zinc-700 border border-zinc-200/80 hover:border-zinc-400 hover:bg-zinc-50 hover:text-zinc-900 rounded-xl text-xs font-bold transition-all shadow-sm">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white/80 backdrop-blur-xl border border-white shadow-[0_8px_30px_rgba(0,0,0,0.04)] rounded-2xl p-6">
            ...
            <div className="space-y-3">
              {projSuites.length === 0 ? (
                <div className="text-center py-10 text-zinc-400 bg-zinc-50/50 rounded-xl border-2 border-dashed border-zinc-200/80 text-[13px] font-medium">
                  <FileSpreadsheet size={32} className="mx-auto mb-3 text-zinc-300 opacity-60" />
                  ...
                  return (
                    <div key={suite.id} className="group bg-white/60 border border-zinc-200/60 rounded-xl p-4 flex justify-between items-center hover:bg-white hover:border-zinc-400 hover:shadow-[0_4px_20px_rgba(24,24,27,0.08)] transition-all duration-300 cursor-pointer"
                      ...
                      <div>
                        ...
                        <p className="text-[11px] font-medium text-zinc-500 mt-1">총 {caseCount}개 케이스</p>
                      </div>
                      <div className="w-8 h-8 rounded-full bg-zinc-50 flex items-center justify-center group-hover:bg-zinc-200/50 transition-colors">
                         <ChevronRight size={16} className="text-zinc-400 group-hover:text-zinc-700" />
                      </div>
                    </div>
                  )
                })
          ...
          <div className="bg-white/80 backdrop-blur-xl border border-white shadow-[0_8px_30px_rgba(0,0,0,0.04)] rounded-2xl p-6">
            ...
            <div className="space-y-3">
              {projRuns.length === 0 ? (
                <div className="text-center py-10 text-zinc-400 bg-zinc-50/50 rounded-xl border-2 border-dashed border-zinc-200/80 text-[13px] font-medium">
                  <PlayCircle size={32} className="mx-auto mb-3 text-zinc-300 opacity-60" />
      ...
      <Layout title="데이터 업로드 및 스위트 생성">
        <div className="max-w-2xl mx-auto mt-6 bg-white/80 backdrop-blur-xl border border-white shadow-[0_8px_30px_rgba(0,0,0,0.04)] rounded-2xl overflow-hidden">
          <div className="p-6 border-b border-zinc-200/50 bg-zinc-50/30 backdrop-blur-md">
            ...
            <p className="text-zinc-500 text-[13px] font-medium mt-1.5">
          ...
          <form onSubmit={handleFileUpload} className="p-8 space-y-6">
            <div>
              <label className="block text-[11px] font-bold text-zinc-500 mb-1.5 ml-1 uppercase tracking-wider">Test Suite Name</label>
            ...
            <div className="bg-zinc-100/50 border border-zinc-200/50 rounded-xl p-4 shadow-sm">
              ...
              <div className="flex gap-2.5 text-[11px] font-mono font-bold text-zinc-600 bg-white/80 backdrop-blur-md p-3 rounded-lg border border-zinc-200/60 overflow-x-auto shadow-sm">
                <span className="text-zinc-800">테스트 케이스 명</span> | <span>테스트 환경</span> | <span>기대 결과</span> | ...
              </div>
            </div>

            <div>
              <label className="block text-[11px] font-bold text-zinc-500 mb-1.5 ml-1 uppercase tracking-wider">File Attachment</label>
              <div className={`relative border-2 border-dashed ${selectedFileName ? 'border-emerald-400/80 bg-emerald-50/50' : 'border-zinc-300/80 hover:border-zinc-500/80 bg-zinc-50/50'} rounded-2xl p-8 text-center transition-all duration-300 group`}>
                ...
                {selectedFileName ? (
                    ...
                    <>
                        <FileSpreadsheet size={36} className="mx-auto mb-3 text-zinc-300 group-hover:text-zinc-500 transition-colors" />
                        <p className="text-zinc-700 text-sm font-bold tracking-tight">클릭하거나 파일을 드래그하여 업로드</p>
                        <p className="text-[11px] font-medium text-zinc-400 mt-1.5">.xlsx, .xls, .csv 지원</p>
                    </>
                )}
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-6 mt-4 border-t border-zinc-100">
              ...
              <button type="submit" className="px-6 py-2.5 rounded-xl text-xs font-bold bg-gradient-to-b from-zinc-700 to-zinc-900 hover:from-zinc-800 hover:to-black text-white shadow-[0_4px_14px_0_rgba(24,24,27,0.39)] hover:shadow-[0_6px_20px_rgba(24,24,27,0.23)] border border-zinc-700/50 transition-all">업로드 및 생성</button>
            </div>
          </form>
        </div>
      </Layout>
    ...
    return (
      <Layout title={`스위트: ${suite?.name}`}>
        <div className="mb-5 flex justify-between items-center">
          <button onClick={() => setCurrentView('project')} className="text-xs text-zinc-500 hover:text-zinc-800 flex items-center gap-1.5 font-bold bg-white/60 backdrop-blur-md px-3 py-2 rounded-lg border border-zinc-200/60 shadow-sm transition-all hover:bg-white hover:shadow-md">
            <ChevronRight className="rotate-180" size={16}/> 프로젝트로 돌아가기
          </button>
          <div className="flex items-center gap-3">
            <span className="text-xs font-medium text-zinc-500">총 <span className="text-zinc-800 font-bold">{cases.length}</span>개의 케이스</span>
            <button onClick={() => { setEditSuiteName(suite?.name || ''); setIsSuiteSettingsOpen(true); }} className="text-zinc-400 hover:text-zinc-800 hover:bg-zinc-200/50 p-1.5 rounded-lg transition-all ml-2">
                <Settings size={18} />
            </button>
          </div>
        </div>
        ...
        {isSuiteSettingsOpen && (
            <div className="mb-6 p-5 bg-white/80 backdrop-blur-xl border border-white shadow-[0_8px_30px_rgba(0,0,0,0.06)] rounded-2xl animate-in fade-in slide-in-from-top-2 duration-300">
                ...
                <form onSubmit={handleUpdateSuite} className="flex items-end gap-3">
                    <div className="flex-1">
                        <label className="block text-[11px] font-bold text-zinc-500 mb-1.5 ml-1 uppercase tracking-wider">Rename Suite</label>
                    ...
                    <button type="button" onClick={() => setIsSuiteSettingsOpen(false)} className="px-4 py-2.5 text-zinc-500 hover:bg-zinc-100 rounded-xl text-xs font-bold transition-all">취소</button>
                </form>
            </div>
        )}
            ...
            <tbody className="divide-y divide-zinc-100/80">
              {cases.map((c, i) => (
                <tr key={c.id} className="hover:bg-zinc-50/50 transition-colors">
                  <td className="px-5 py-3.5 text-[11px] font-mono font-bold text-zinc-400 text-center">C{i+1}</td>
                  {suite?.headers?.map((header, idx) => (
                    <td key={idx} className="px-5 py-3.5 text-[13px] font-medium text-zinc-700 max-w-[250px] truncate" title={c.fields?.[header] || ''}>
                      {c.fields?.[header] || '-'}
                    </td>
                  ))}
                </tr>
              ...
              {cases.length === 0 && (
                <tr><td colSpan={suite?.headers ? suite.headers.length + 1 : 4} className="p-10 text-center text-[13px] font-medium text-zinc-400">케이스가 없습니다.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="max-w-4xl mx-auto mt-6 bg-white/80 backdrop-blur-xl border border-white shadow-[0_8px_30px_rgba(0,0,0,0.04)] rounded-2xl p-8">
          <form onSubmit={handleCreateRun}>
            <div className="mb-8">
              ...
              <input name="runName" required defaultValue={`Sprint Release Run - ${new Date().toLocaleDateString()}`} className="w-full bg-white/50 backdrop-blur-sm border border-zinc-200/80 rounded-xl px-4 py-3 text-sm font-bold text-zinc-900 focus:outline-none focus:border-emerald-500/50 focus:ring-4 focus:ring-emerald-500/10 transition-all shadow-inner" />
            </div>
            
            <div className="mb-8">
              <div className="flex justify-between items-end mb-3 px-1">
                <h4 className="text-[13px] font-bold text-zinc-700">포함할 테스트 케이스 선택</h4>
              ...
              <div className="bg-white/50 border border-zinc-200/80 rounded-xl max-h-[500px] overflow-y-auto custom-scrollbar shadow-inner">
                {projSuites.map(suite => {
                  ...
                  const isCollapsed = collapsedSuites.includes(suite.id);
                  ...
                  return (
                    <div key={suite.id} className="border-b border-zinc-200/60 last:border-0">
                      ...
                      <div className="flex items-center gap-3 p-4 bg-zinc-50/80 backdrop-blur-md border-b border-zinc-200/60 sticky top-0 z-10 cursor-pointer hover:bg-zinc-100/80 transition-colors" onClick={() => handleCollapseToggle(suite.id)}>
                        ...
                        <span className="text-[13px] font-black text-zinc-800">{suite.name}</span>
                        <span className="text-[11px] font-bold text-zinc-500 ml-auto bg-white/80 px-2.5 py-1 rounded-md border border-zinc-200/50 shadow-sm">{checkedCasesInSuite.length} / {suiteCases.length}</span>
                      </div>
                      
                      {!isCollapsed && (
                        <div className="divide-y divide-zinc-100/80 bg-white/40">
                          {suiteCases.map(c => (
                            <label key={c.id} className="flex items-start gap-4 p-4 hover:bg-white hover:shadow-sm cursor-pointer transition-all duration-300 group">
                              <div className="mt-0.5 shrink-0">
                                <input type="checkbox" checked={selectedRunCases.includes(c.id)} onChange={() => handleCaseToggle(c.id)} className="w-4 h-4 text-emerald-500 border-zinc-300 rounded focus:ring-emerald-500/20 cursor-pointer transition-all" />
                              </div>
                              <div className="flex-1 min-w-0">
                                <p className="text-[13px] font-bold text-zinc-800 mb-2 group-hover:text-emerald-700 transition-colors tracking-tight">{c.title}</p>
                                <div className="flex flex-wrap gap-x-6 gap-y-2">
                                  {suite.headers.slice(0, 3).map(h => {
                                      if(!c.fields[h] || c.fields[h] === c.title) return null;
                                      return (
                                        <span key={h} className="text-[11px] font-medium text-zinc-500 truncate max-w-[300px]">
                                          <span className="font-bold text-zinc-400 mr-1.5">{h}:</span>{c.fields[h]}
                                        </span>
                                      );
                                  })}
                                </div>
                ...
                {projSuites.length === 0 && <p className="text-[13px] font-medium text-zinc-400 p-10 text-center">사용 가능한 스위트가 없습니다.</p>}
              </div>
            </div>
      ...
      <Layout title={`실행: ${run.name}`}>
        <div className="mb-4 flex justify-between items-center">
          ...
          {run.status === 'active' ? (
            ...
            <span className="px-3 py-1.5 rounded-lg text-xs font-bold bg-zinc-100/80 text-zinc-500 border border-zinc-200/80 shadow-inner">완료된 런</span>
          )}
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl border border-white shadow-[0_8px_30px_rgba(0,0,0,0.04)] rounded-2xl p-5 mb-5 flex flex-col gap-3">
           <div className="flex justify-between items-center">
              ...
              <div className="flex gap-5 text-[11px] font-bold">
                 ...
                 <span className="text-zinc-400">UNTESTED: {total - pCount - fCount - bCount}</span>
              </div>
           </div>
           <div className="h-4 rounded-full overflow-hidden shadow-inner border border-zinc-100/50 bg-zinc-50">
             <ResponsiveContainer width="100%" height="100%">
               <BarChart layout="vertical" data={[{name: 'progress', PASS: pCount, FAIL: fCount, BLOCK: bCount, UNTESTED: total - (pCount+fCount+bCount)}]} margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                 ...
                 <Bar dataKey="UNTESTED" stackId="a" fill="#e4e4e7" />
                 <defs>
          ...
          <div className="flex-1 flex flex-col bg-white/80 backdrop-blur-xl border border-white shadow-[0_8px_30px_rgba(0,0,0,0.04)] rounded-2xl overflow-hidden transition-all min-w-0">
            <div className="p-4 border-b border-zinc-200/60 bg-zinc-50/50 backdrop-blur-md flex justify-between items-center relative z-20">
              <div className="relative">
                 ...
                 {isHeaderDropdownOpen && (
                    <div className="absolute top-full mt-2 left-0 z-50 bg-white/90 backdrop-blur-xl border border-zinc-200/80 shadow-[0_10px_40px_rgba(0,0,0,0.1)] rounded-xl p-3 w-56 max-h-56 overflow-y-auto">
                       <p className="text-[10px] font-bold text-zinc-400 mb-2 uppercase tracking-wider ml-1">목록 표시 항목</p>
                       {runHeaders.map(h => (
                          <label key={h} className="flex items-center gap-2.5 py-1.5 px-1 hover:bg-zinc-50 rounded-lg cursor-pointer transition-colors">
                             ...
                             }} className="w-3.5 h-3.5 text-zinc-600 rounded border-zinc-300 focus:ring-zinc-500/20" />
                             <span className="text-[12px] font-bold text-zinc-700 truncate">{h}</span>
                          </label>
                       ))}
                    </div>
                 )}
              ...
              <div className="flex bg-zinc-200/50 p-1 rounded-lg">
                {['all', 'untested', 'pass', 'fail', 'block'].map(f => (
                  <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1.5 text-[10px] font-black uppercase rounded-md transition-all duration-300 ${filter === f ? 'bg-white text-zinc-800 shadow-sm' : 'text-zinc-500 hover:text-zinc-800'}`}>
                    {f === 'all' ? '전체' : f}
                  </button>
                ))}
              </div>

              <button onClick={() => setIsDetailOpen(!isDetailOpen)} className="text-[11px] px-3 py-2 bg-zinc-100/80 text-zinc-700 border border-zinc-200/60 rounded-lg hover:bg-zinc-200 flex items-center gap-1.5 font-bold ml-2 transition-all">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar bg-white/40">
              {filteredCases.map((c, idx) => {
                ...
                let icon = <div className="w-3.5 h-3.5 rounded-full border-2 border-zinc-300"></div>;
                ...
                return (
                  <div key={c.id} 
                    className={`p-4 border-b border-zinc-100/80 transition-all duration-300 flex flex-col gap-3 last:border-0 ${isSelected ? 'bg-zinc-100/40 border-l-4 border-l-zinc-700' : 'hover:bg-zinc-50/60 border-l-4 border-l-transparent'}`}>
                    ...
                    <div className="flex items-start justify-between gap-4 cursor-pointer" onClick={() => { setSelectedCaseId(c.id); if(!isDetailOpen) setIsDetailOpen(true); }}>
                      <div className="flex items-start gap-3 flex-1 min-w-0 mt-0.5">
                        ...
                        <p className={`text-[13px] tracking-tight truncate ${isSelected ? 'text-zinc-900 font-black' : 'text-zinc-800 font-bold'}`}>{c.title}</p>
                      </div>
                      
                      <div className="flex gap-1.5 shrink-0 bg-white/80 backdrop-blur-sm rounded-lg border border-zinc-200/80 p-1 shadow-sm" onClick={(e) => e.stopPropagation()}>
                         <button onClick={(e) => { e.stopPropagation(); handleInlineResultUpdate(c.id, 'pass'); }} title="PASS" className={`p-1.5 rounded-md transition-all ${status === 'pass' ? 'bg-gradient-to-b from-emerald-400 to-emerald-500 text-white shadow-md border-emerald-400' : 'text-zinc-400 hover:bg-emerald-50 hover:text-emerald-500'}`}>
                           <CheckCircle size={14}/>
                         </button>
                         <button onClick={(e) => { e.stopPropagation(); handleInlineResultUpdate(c.id, 'fail'); }} title="FAIL" className={`p-1.5 rounded-md transition-all ${status === 'fail' ? 'bg-gradient-to-b from-rose-400 to-rose-500 text-white shadow-md border-rose-400' : 'text-zinc-400 hover:bg-rose-50 hover:text-rose-500'}`}>
                           <XCircle size={14}/>
                         </button>
                         <button onClick={(e) => { e.stopPropagation(); handleInlineResultUpdate(c.id, 'block'); }} title="BLOCK" className={`p-1.5 rounded-md transition-all ${status === 'block' ? 'bg-gradient-to-b from-amber-400 to-amber-500 text-white shadow-md border-amber-400' : 'text-zinc-400 hover:bg-amber-50 hover:text-amber-500'}`}>
                        ...
                        <div className="pl-6 pt-1 cursor-pointer" onClick={() => { setSelectedCaseId(c.id); if(!isDetailOpen) setIsDetailOpen(true); }}>
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            {selectedHeaders.map(h => {
                              ...
                              return (
                                  <div key={h} className="bg-white border border-zinc-200/60 rounded-xl p-3 shadow-sm hover:shadow-md transition-shadow">
                                    <div className="font-black text-[10px] text-zinc-500 mb-1.5 uppercase tracking-wider">{h}</div>
                                    <div className="text-[12px] font-medium text-zinc-700 whitespace-pre-wrap leading-relaxed">{val}</div>
                                  </div>
                              )
                            })}
                          </div>
              ...
              {filteredCases.length === 0 && <p className="text-center text-zinc-400 text-[13px] font-bold mt-10">해당하는 케이스가 없습니다.</p>}
            </div>
          </div>
            ...
            <div className="w-1/3 min-w-[340px] flex flex-col bg-white/80 backdrop-blur-xl border border-white shadow-[0_8px_30px_rgba(0,0,0,0.04)] rounded-2xl overflow-hidden transition-all duration-500">
              {run.status === 'completed' && (!selectedCase || !isDetailOpen) ? (
                <div className="flex-1 p-8 flex flex-col items-center justify-center bg-zinc-50/50">
                  ...
                  <h2 className="text-xl font-black text-zinc-800 mb-8 tracking-tight">테스트 런 완료 보고서</h2>
                  <div className="flex gap-4 mb-8 bg-white/80 backdrop-blur-md p-6 rounded-2xl border border-white shadow-[0_8px_30px_rgba(0,0,0,0.06)] w-full justify-between">
                    <div className="text-center flex-1">
                      <p className="text-emerald-500 font-black text-3xl mb-1">{pCount}</p>
                      <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Pass</p>
                    </div>
                    <div className="w-px bg-zinc-200"></div>
                    <div className="text-center flex-1">
                      <p className="text-rose-500 font-black text-3xl mb-1">{fCount}</p>
                      <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Fail</p>
                    </div>
                    <div className="w-px bg-zinc-200"></div>
                    <div className="text-center flex-1">
                      <p className="text-amber-500 font-black text-3xl mb-1">{bCount}</p>
                      <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Block</p>
                    </div>
                  </div>
                </div>
              ) : selectedCase && selectedResult ? (
                <>
                  <div className="p-6 border-b border-zinc-200/60 bg-white/60 backdrop-blur-md">
                    <div className="flex justify-between items-start mb-4">
                      <h2 className="text-[15px] font-black text-zinc-900 leading-snug tracking-tight">{selectedCase.title}</h2>
                      <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold border shrink-0 ${selectedCase.priority === 'Critical' ? 'border-rose-200/80 text-rose-600 bg-rose-50/80' : 'border-zinc-200/80 text-zinc-500 bg-zinc-50/80'}`}>
                        {selectedCase.priority}
                      </span>
                    </div>
                    
                    <div className="flex gap-2.5 mt-5">
                      <button onClick={() => handleResultUpdate('pass')} className={`flex-1 py-2.5 rounded-xl flex items-center justify-center gap-2 border transition-all duration-300 ${selectedResult.status === 'pass' ? 'bg-gradient-to-b from-emerald-400 to-emerald-500 border-emerald-400 text-white shadow-[0_6px_20px_rgb(16,185,129,0.3)]' : 'border-zinc-200/80 bg-white/60 text-zinc-600 hover:border-emerald-300 hover:bg-emerald-50/50 hover:text-emerald-600'}`}>
                        <CheckCircle size={16} />
                        <span className="text-[11px] font-black tracking-widest uppercase">Pass</span>
                      </button>
                      <button onClick={() => handleResultUpdate('fail')} className={`flex-1 py-2.5 rounded-xl flex items-center justify-center gap-2 border transition-all duration-300 ${selectedResult.status === 'fail' ? 'bg-gradient-to-b from-rose-400 to-rose-500 border-rose-400 text-white shadow-[0_6px_20px_rgb(244,63,94,0.3)]' : 'border-zinc-200/80 bg-white/60 text-zinc-600 hover:border-rose-300 hover:bg-rose-50/50 hover:text-rose-600'}`}>
                        <XCircle size={16} />
                        <span className="text-[11px] font-black tracking-widest uppercase">Fail</span>
                      </button>
                      <button onClick={() => handleResultUpdate('block')} className={`flex-1 py-2.5 rounded-xl flex items-center justify-center gap-2 border transition-all duration-300 ${selectedResult.status === 'block' ? 'bg-gradient-to-b from-amber-400 to-amber-500 border-amber-400 text-white shadow-[0_6px_20px_rgb(245,158,11,0.3)]' : 'border-zinc-200/80 bg-white/60 text-zinc-600 hover:border-amber-300 hover:bg-amber-50/50 hover:text-amber-600'}`}>
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar bg-zinc-50/30">
                    {(() => {
                      ...
                      if (caseSuite?.headers && selectedCase.fields) {
                        return caseSuite.headers.map((header, idx) => {
                          ...
                          return (
                            <section key={idx} className="bg-white/80 backdrop-blur-md p-4 rounded-xl border border-white shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
                              <h4 className="text-[10px] font-black text-zinc-400 mb-2 uppercase tracking-wider">{header}</h4>
                              <div className="text-zinc-800 text-[13px] font-medium whitespace-pre-wrap leading-relaxed">
                                {val}
                              </div>
                            </section>
                          );
                    ...
                    <section className="bg-white/80 backdrop-blur-md p-4 rounded-xl border border-white shadow-[0_2px_10px_rgba(0,0,0,0.02)] mt-4">
                      <h4 className="text-[10px] font-black text-zinc-400 mb-2 uppercase tracking-wider">실제 결과 및 메모 (Notes)</h4>
                      <textarea 
                        value={selectedResult.note || ''} 
                        onChange={handleNoteUpdate}
                        placeholder="버그 정보, 환경, 이슈 링크 등을 기록하세요."
                        className="w-full bg-zinc-50/80 border border-zinc-200/60 rounded-xl p-3 text-zinc-800 text-[13px] font-medium focus:border-zinc-500/50 focus:ring-4 focus:ring-zinc-500/10 outline-none transition-all resize-none h-28 shadow-inner"
                      />
                    </section>
                  </div>
                </>
              ) : (
                <div className="flex-1 flex items-center justify-center text-zinc-400 flex-col bg-zinc-50/50">
                  <ShieldAlert size={40} className="mb-4 text-zinc-300 opacity-50" />
  ...
  return <div className="bg-[#f4f4f5] h-screen flex items-center justify-center text-zinc-500 text-sm font-bold tracking-widest">로딩 중...</div>;
}
